...
from models_resnet import Resnet18_md, Resnet50_md, ResnetModel, MonodepthModel
...
def get_model(model, input_channels=3, pretrained=False):
    ...
    elif model == 'MonodepthModel':
        out_model == MonodepthModel(input_channels)
    else:
...
def prepare_dataloader(data_directory, mode, augment_parameters,
    ...
    '''
    data_directory: 传入数据的路径
    mode: 传入模式
    augment_parameters: 六个数的列表
    do_augmentation: bool值
    batch_size: 自行指定
    size: 图片大小
    num_workers: 工作线程数
    '''
