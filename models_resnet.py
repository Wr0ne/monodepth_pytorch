...
class MonodepthModel(nn.Module):
    def __init__(self, num_in_layers):
        ...
        # encoder
        self.conv1 = convblock(num_in_layers, 32, 7)
        self.conv2 = convblock(32, 64, 5)
        self.conv3 = convblock(64, 128, 3)
        self.conv4 = convblock(128, 256, 3)
        self.conv5 = convblock(256, 512, 3)
        self.conv6 = convblock(512, 512, 3)
        self.conv7 = convblock(512, 512, 3)

        # decoder
        self.upconv7 = upconv(512, 512, 3, 2)
        self.iconv7 = conv(512+512, 512, 3, 1) # conv6 + upconv7
        self.upconv6 = upconv(512, 512, 3, 2)
        self.iconv6 = conv(512+512, 512, 3, 1) # conv5 + upconv6
        self.upconv5 = upconv(512, 256, 3, 2)
        self.iconv5 = conv(512, 256, 3, 1) # conv4 + upconv5

        # 后四张图产生视差
        self.upconv4 = upconv(256, 128, 3, 2)
        self.iconv4 = conv(256, 128, 3, 1) # conv3 + upconv4
        self.disp4_layer = get_disp(128) # layer 4

        self.upconv3 = upconv(128, 64, 3, 2)
        self.iconv3 = conv(64+64+2, 64, 3, 1) # conv2 + upconv3 + layer4
        self.disp3_layer = get_disp(64) # layer 3

        self.upconv2 = upconv(64, 32, 3, 2)
        self.iconv2 = conv(32+32+2, 32, 3, 1) # conv1 + upconv2 + layer3
        self.disp2_layer = get_disp(32) # layer 2

        self.upconv1 = upconv(32, 16, 3, 2)
        self.iconv1 = conv(16+2, 16, 3, 1) # upconv1 + layer2
        self.disp1_layer = get_disp(16) # layer 1
    ...
    def forward(self, x):
        ...
        # decoder
        up7 = self.upconv7(x7)
        cat7 = torch.cat((up7, sk6), 1)
        ic7 = self.iconv7(cat7)

        up6 = self.upconv6(ic7)
        cat6 = torch.cat((up6, sk5), 1)
        ic6 = self.iconv6(cat6)

        up5 = self.upconv5(ic6)
        cat5 = torch.cat((up5, sk4), 1)
        ic5 = self.iconv5(cat5)

        up4 = self.upconv4(ic5)
        cat4 = torch.cat((up4, sk3), 1)
        ic4 = self.iconv4(cat4)
        self.disp4 = self.disp4_layer(ic4)
        self.udisp4 = F.interpolate(self.disp4, scale_factor = 2, mode='bilinear', align_corners=True)

        up3 = self.upconv3(ic4)
        cat3 = torch.cat((up3, sk2, self.udisp4), 1)
        ic3 = self.iconv3(cat3)
        self.disp3 = self.disp3_layer(ic3)
        self.udisp3 = F.interpolate(self.disp3, scale_factor = 2, mode='bilinear', align_corners=True)

        up2 = self.upconv2(ic3)
        cat2 = torch.cat((up2, sk1, self.udisp3), 1)
        ic2 = self.iconv2(cat2)
        self.disp2 = self.disp2_layer(ic2)
        self.udisp2 = F.interpolate(self.disp2, scale_factor = 2, mode='bilinear', align_corners=True)

        up1 = self.upconv1(ic2)
        cat1 = torch.cat((up1, self.udisp2), 1)
        ic1 = self.iconv1(cat1)
        self.disp1 = self.disp1_layer(ic1)
        return self.disp1, self.disp2, self.disp3, self.disp4
        # 返回需要的四张视差图
...
if __name__ == '__main__':
    model = MonodepthModel(3)
    img = torch.zeros((1, 3, 256, 512))
    layer1, layer2, layer3, layer4 = model(img)
